import { balancedFocus, initialCeoPersonal, initialCeoPortfolio, buildInitialBoard, fnv1a, generateMaTargets, initialCompetitorStrikeState, initialCrisisState, initialPoliticsState, initialInsuranceState, initialCertificationState, initialProductModules, initialFiskusState, initialMacroState, initialIpoState, initialLaborState, initialLegalState, initialTakeoverState, personaBits, resolveLocationProfile, stream, type CompanyState } from '@boardroom/shared';
...
export function ensureStateShape(state: CompanyState): CompanyState {
  ...
  if (!s.fiskus) s.fiskus = initialFiskusState();
