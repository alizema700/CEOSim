...
import {
  ...
  regulationSummaryDe,
  insuranceSummaryDe,
  certificationSummaryDe,
  productStudioSummaryDe,
  fiskusSummaryDe,
  laborSummaryDe,
  legalSummaryDe,
  runwayWeeks,
...
export function stateBriefDe(state: CompanyState): string {
  ...
  return [
    ...
    `Versicherungen: ${insuranceSummaryDe(state)}.`,
    `Zertifizierungen: ${certificationSummaryDe(state)}.`,
    ...(productStudioSummaryDe(state) ? [`Produkt-Studio: ${productStudioSummaryDe(state)}.`] : []),
    ...(state.fiskus && (state.fiskus.strategy !== 'konservativ' || state.fiskus.activeAudit || state.fiskus.finesTotal > 0) ? [`⚠️ Fiskus: ${fiskusSummaryDe(state)}.`] : []),
    ...
    `Ausblick: ${outlookSummaryDe(state)}`,
